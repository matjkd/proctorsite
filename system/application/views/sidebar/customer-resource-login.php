<div class="sidebox">

	<div style="width:318px; float:left;">
		
		<a style="float:left" id="desc2" title="All of the resources our customers need: Interactive Help & User Guides, Support Call Logging and tracking, alongside powerful financial calculators & business modelling tools." >
			<img width="200px" height="21px" src="<?=base_url()?>images/white_headers/customer-resource.png"/>
		</a>


		<div class="clear"></div>
		<form method="post" action="http://www.customer-resource.com/user/login/validate_credentials"  target="_blank" >
			
		 <div id="inputs"> 
			   <input type="text" id="username" name="username" title="Username" />  
			
			   
			   <input type="password" id="password" name="password" title="Password" />
		</div>
	</div>
	
	<div style="width:41px; padding-top:5px; float:left;">
		
		<input type="image" name="submit" src="<?=base_url()?>images/icons/redbook_icon.png" border="0" />
		</form>
	</div>
</div>

